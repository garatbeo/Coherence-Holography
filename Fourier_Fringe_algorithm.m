...
%% Generate complex  sub-objects with amplitude and phase
...
subobject{1}=GT1_abs.*exp(1i*GT1_phase);

GT2_abs=(GT2_abs+1)/2*255; %8-bit value
GT2_phase=(GT2_phase+1)/2*pi;%0-pi value
subobject{2}=GT2_abs.*exp(1i*GT2_phase);

GT3_abs=(GT3_abs+1)/2*255; %8-bit value
GT3_phase=(GT3_phase+1)/2*pi;%0-pi value
subobject{3}=GT3_abs.*exp(1i*GT3_phase);

%% Apply equation 1 to find the object spectrum

z=deltaz_a(1);
pro=exp(-1i*kz*z); 
G1=pro.*(fftshift(fft2(fftshift(subobject{1}))))*dx*dx; %1st object wave at hologram plane

z=deltaz_a(2);  
pro=exp(-1i*kz*z);
G2=pro.*(fftshift(fft2(fftshift(subobject{2}))))*dx*dx;%1st object wave at hologram plane

z=deltaz_a(3);
pro=exp(-1i*kz*z);
G3=pro.*(fftshift(fft2(fftshift(subobject{3}))))*dx*dx;%1st object wave at hologram plane

G=G1+G2+G3; %total object wave at the hologram plane
imagesc(fx,fy,abs(G))
xlabel('1/mm')
ylabel('1/mm')
...
n_subObj=2;%index of the subobject that wants to be reconstucted
